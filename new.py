...
def verifypassword(string,hashvalue):
    if joinstr(str(string))==hashvalue:
        return True
    else :
        return False
...
class Login:
    def __init__(self,root):
        ...
        self.root.geometry("1184x780+100+50")
        ...
        new_user_btn=Button(Frame_login,text="New User? Sign In",bg="white",fg="#d77337",bd=0,font=("times new roman",12),command=self.forget).place(x=90,y=280)
        login_btn=Button(self.root,text="Login",fg="white",bg="#d77337",font=("times new roman",20),command=self.loginfunc).place(x=300,y=470,width=180,height=40)
    ...
    def forget(self):
        ...
        title=Label(Frame_login,text="New User Sign In",font=("Impact",35,"bold"),fg="#055BB5",bg="white").place(x=90,y=5)
        ...
        lbl_pass=Label(Frame_login,text="Password",font=("Goudy old style",13,"bold"),fg="gray",bg="white").place(x=90,y=150)

        self.passWord=Entry(Frame_login,font=("times new roman",15),bg="lightgray")
        self.passWord.place(x=90,y=180,width=350,height=35)
        lbl_passcon=Label(Frame_login,text="Confirm Password",font=("Goudy old style",13,"bold"),fg="gray",bg="white").place(x=90,y=220)

        self.passConf=Entry(Frame_login,font=("times new roman",15),bg="lightgray")
        self.passConf.place(x=90,y=250,width=350,height=35)
        reg_btn=Button(self.root1,text="Register",fg="white",bg="#055BB5",font=("times new roman",20),command=self.regfunc).place(x=170,y=300,width=180,height=40)

        
    def regfunc(self):
        usernn=self.usern.get()
        passWordd=self.passWord.get()
        passConff=self.passConf.get()
        if passWordd==passConff:
            try:
                registration(usernn,passWordd)
                messagebox.showinfo(title='Info', message="Successfully Registered!!")
            except:
                messagebox.showerror(title='Error', message="Failed to Register!!")
        else:
            messagebox.showerror(title='Error', message="Failed to Register!!")
    def loginfunc(self):
